---
name: Wasm-Ai
description: Vercel and web-llm template to run wasm models directly in the browser.
---

# Wasm-Ai

Vercel and web-llm template to run wasm models directly in the browser.

## How to Use

Visit the official resource: [https://github.com/hrishioa/wasm-ai](https://github.com/hrishioa/wasm-ai)

### Installation

If this is a GitHub repository, you can typically clone and install it:

```bash
git clone https://github.com/hrishioa/wasm-ai
cd wasm-ai
# Follow the repository's README for specific installation instructions
```

### Getting Started

Please refer to the official documentation and repository for detailed setup and usage instructions.


## Resources

- Official Link: [https://github.com/hrishioa/wasm-ai](https://github.com/hrishioa/wasm-ai)
